getBelehradekConstants<-function(sources="all",
                                 stages="all"){
  dfr<-rbind(
        data.frame(source="Yamamoto et al. 2014",stage="Z1",a= 229.5, se_a= 11.05,b= 0.63,se_b=0.26, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="Z1-M", a= 530.5, se_a= 22.38,b=-0.02,se_b=0.26, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="M", a= 417.3, se_a=  0.09,b=-2.24,se_b=0.09, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C1",a= 276.07,se_a= 15.93,b=-1.29,se_b=0.14, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C2",a= 429.06,se_a= 18.99,b=-2.54,se_b=0.19, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C3",a= 511.44,se_a= 33.06,b=-2.63,se_b=0.28, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C4",a= 762.62,se_a= 74.77,b=-3.68,se_b=0.58, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C5",a= 801.73,se_a= 98.33,b=-3.28,se_b=0.67, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C6",a= 996.53,se_a=133.14,b=-4.87,se_b=0.95, c=1.0),
        data.frame(source="Yamamoto et al. 2014",stage="C7",a=1115.32,se_a=177.68,b=-4.02,se_b=0.10, c=1.0),
        data.frame(source="Ouellet and Ste Marie 2018",stage="Z1",a=4903.33,se_a=0, b=-6.1 ,se_b=0, c=1.87),
        data.frame(source="Ouellet and Ste Marie 2018",stage="Z2",a=2461.31,se_a=0, b=-4.73,se_b=0 ,c=1.67),
        data.frame(source="Ouellet and Ste Marie 2018",stage="M" ,a= 260.39,se_a=0 ,b=-1.37,se_b=0, c=0.82)
       );
...
#' to S [(T-b)^c] dt = a. Note that Reamur's Law can be obtained by setting the exponent "c" to 1.
#'
#' @export
#'
  ...
  for (source in sources){
    for (stage in stages){
      ...
      if (!is.null(cs)){
        ...
        dfr<-rbind(dfr,data.frame(source=source,stage=stage,T=T,D=D));
      }
    }
  }
  return(dfr);
...
#'
#' @title Calculate the IMD by stage using Ouellet and Ste. Marie (2018) for fixed T
#'
#' @description Function to calculate the IMD by stage using Ouellet and Ste. Marie (2018) for fixed T.
#'
...
#'
#' @return dataframe with stage, temperature, and intermolt duration, in the time units of \code{a}.
#'
...
#' See also \code{calcIntermoltDuration_Belehradek}.
#'
#' @export
#'
calcIMDbyStage_FixedT<-function(T,
  ...
  for (stage in stages){
    ...
    D <- cs$a/(T-cs$b)^cs$c;
    idx<-D<0; D[idx]<-NA;
    dfr<-rbind(dfr,data.frame(stage=stage,T=T,D=D,stringsAsFactors=FALSE));
  }
  return(dfr);
}
...
#' @return The intermolt duration, in the time units of \code{a}.
#'
#' @details The intermolt duration using Belehradek's function is D = a/(T-b)^c for development at constant T, so
#' T = (a/D)^(1/c)+b for a given intermolt duration..
#'
#' @export
#'
...
testIMD_Belehradek<-function(Ts,
                             sources="all",
                             stages=c("Z1","Z2","M"),
                             lims=NULL){
  dfr<-NULL;
